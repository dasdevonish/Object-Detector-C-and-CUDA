...
void HOG_function_train(IplImage *input, int mode);
...
void write_label(int HOG_label, int mode);
void train_svm(void);
void accuracy_svm(void);
...
void enterGPU(IplImage **input, int mode, struct svm_model* model, int total_num_of_windows, int *window_array, int *p_prediction_count, int windowSize, double scale, short **position_window_x, short **position_window_y, box *b_boxes);
int my_SVM_prediction(double *HOG_features, struct svm_model* model, double *prob_estimates);
double my_svm_predict(struct svm_model* model, struct svm_node *x);
double my_svm_predict_probability(struct svm_model *model, struct svm_node *x, double *prob_estimates);
...
void store_bounding_box_points(int windowSize, double scale, int pyramid_num, int position_window_x, int position_window_y, box *b_boxes, int *p_prediction_count, double *prob_estimates);
void draw_bounding_box(IplImage *output, box *b_boxes, int prediction_count);
bool *nms(box *b_boxes, int count);
...
void write_label(int HOG_label, int mode)
{
	...
	{
		FILE *fptr;
		fptr = fopen("data\\SVM_Format\\ObjectDetectionAccuracy.svm", "a");
		if (fptr == NULL)
		{
			printf("Error!");
			exit(1);
...
void train_svm(void)
{
	...
	system("External_Libraries\\libsvm\\windows\\svm-train.exe -b 1 data\\SVM_Format\\ObjectDetectionTrain.svm data\\Models\\ObjectDetectionModel");
...
void accuracy_svm(void)
{
	printf("Accuracy file with HOG features created! \nGetting SVM Model accuracy...\n");
	system("External_Libraries\\libsvm\\windows\\svm-predict.exe data\\SVM_Format\\ObjectDetectionAccuracy.svm data\\Models\\ObjectDetectionModel data\\Models\\Accuracy");
	//printf("Model successfully created!\n");
	...
	if (p_dir)
	{
		while ((entry = readdir(p_dir)) != NULL)
		{
			if (entry->d_type == DT_REG)
			{
				...
				box *b_boxes;//bounding box locations

				int prediction_count = 0;
				int max_prediction_count = 1000;
				...
				b_boxes = (box *)malloc(max_prediction_count * sizeof(box));
				...
				int i;
				for (i = 0; i < pyramid_iteration + 1; i++)
				{
				...
				enterGPU(input_all_images, mode, model, total_num_of_windows, window_array, p_prediction_count, windowSize, scale, position_window_x, position_window_y, b_boxes);

				printf("Prediction count: %u\n", prediction_count);
				draw_bounding_box(output, b_boxes, prediction_count);
...
void enterGPU(IplImage **input, int mode, struct svm_model* model, int total_num_of_windows, int *window_array, int *p_prediction_count, int windowSize, double scale, short **position_window_x, short **position_window_y, box *b_boxes)
{
	...
	for (i = 0; i < total_num_of_windows; i++)
	{
		...
		if (i < total_num_of_windows)
		{
			...
			int prediction_num = my_SVM_prediction(HOG_features, model, prob_estimates);
			free(HOG_features);
			...
			if (prediction_num > 0)
			{
				...
				store_bounding_box_points(windowSize, scale, first_num, position_window_x[first_num][second_num], position_window_y[first_num][second_num], b_boxes, p_prediction_count, prob_estimates);
			}
...
{ 
	...
	predict_label = my_svm_predict_probability(model, x, prob_estimates);
...
double my_svm_predict_probability(struct svm_model *model, struct svm_node *x, double *prob_estimates)
{
	...
	double pseudo_pred_result = my_svm_predict_values(model, x, dec_values);

	double min_prob = 1e-7;
	double **pairwise_prob = (double **)malloc(nr_class * sizeof(double *));
	for (i = 0; i < nr_class; i++)
		pairwise_prob[i] = (double *)malloc(nr_class * sizeof(double));
	int k = 0;
	for (i = 0; i < nr_class; i++)
...
void store_bounding_box_points(int windowSize, double scale,  int pyramid_num, int position_window_x, int position_window_y, box *b_boxes, int *p_prediction_count, double *prob_estimates)
{
	...
	b_boxes[prediction_count].x = (int)(position_window_x * (scale_multiple));
	b_boxes[prediction_count].y = (int)(position_window_y * (scale_multiple));
	b_boxes[prediction_count].w = (int)(windowSize * (scale_multiple));
	b_boxes[prediction_count].h = (int)(windowSize * (scale_multiple));
	b_boxes[prediction_count].s = prob_estimates[0];
	
	printf("p1.x: %u, p1.y: %u\n", b_boxes[prediction_count].x, b_boxes[prediction_count].y);
}
void draw_bounding_box(IplImage *output, box *b_boxes, int prediction_count)
{
	CvPoint temp1;
	...
	int i;
	bool *res = nms(b_boxes, prediction_count);
	for (i = 1; i < prediction_count + 1; i++)
	{
		if (res[i] == false)
		{
			//printf("Results= %d", i);
			temp1.x = b_boxes[i].x;
			temp1.y = b_boxes[i].y;
			temp2.x = b_boxes[i].x + b_boxes[i].w;
			temp2.y = b_boxes[i].y + b_boxes[i].h;
			cvRectangle(output, temp1, temp2, CV_RGB(255, 0, 0), 2, 8, 0);
		}
	}	
}
bool *nms(box *b_boxes, int count)
{
	...
	bool *res = (bool*)malloc(count * sizeof(bool));
	int i;
	int j;
	for(i = 1; i < count + 1; i++)
		res[i] = false;
	...
	for (i = 1; i < count + 1; i++)
	{
		for (j = 1; j < count + 1; j++)
		{
			if (b_boxes[i].s < b_boxes[j].s)
			{
				a = (float)(b_boxes[j].x + 1)*(b_boxes[j].y + 1);
				w = (float)max((float)0, (min((b_boxes[i].x + b_boxes[i].w), (b_boxes[j].x + b_boxes[j].w)) - max(b_boxes[i].x, b_boxes[j].x)));
				h = (float)max((float)0, min((b_boxes[i].y + b_boxes[i].h), (b_boxes[j].y + b_boxes[j].h)) - max(b_boxes[i].y, b_boxes[j].y));
				if ((w*h) / a > theta && b_boxes[j].w > 0)
				{
					printf("\n%d--%d\n", i, j);
					res[i] = true;
				}
...
int main()
{
	...
	else if (mode == 2)
	{
		...
		char *path1 = "data\\Objects\\positive_examples(+accuracy)";
		path[0] = &path1;
		char *path2 = "data\\Objects\\negative_examples(+accuracy)";
		path[1] = &path2;
		//remove("data\\SVM_Format\\ObjectDetectionAccuracy.svm");
		//create_dataset_in_svm_format(path, num_of_folders, mode);
		/*Accuracy SVM machine*/
		accuracy_svm();
	}
	else if (mode == 3)
	{
